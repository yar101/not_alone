...
class ServiceModerationController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        $rawCounts = Service::query()
            ->selectRaw('status, (resubmitted_at IS NOT NULL) as is_resubmitted, count(*) as count')
            ->groupBy('status', DB::raw('(resubmitted_at IS NOT NULL)'))
            ->get();

        $counts = [
            'pending' => (int) ($rawCounts->first(fn ($r) => $r->status === 'pending' && ! $r->is_resubmitted)?->count ?? 0),
            'approved' => (int) ($rawCounts->where('status', 'approved')->sum('count')),
            'rejected' => (int) ($rawCounts->where('status', 'rejected')->sum('count')),
            'has_remarks' => (int) ($rawCounts->where('status', 'has_remarks')->sum('count')),
            'resubmitted' => (int) ($rawCounts->first(fn ($r) => $r->status === 'pending' && $r->is_resubmitted)?->count ?? 0),
        ];
