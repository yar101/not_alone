...
function submit() {
    if (editingId.value) {
        form.patch(route('admin.traits.update', editingId.value), {
            preserveScroll: true,
            onSuccess: closeForm,
        });
    } else {
        form.post(route('admin.traits.store'), {
            preserveScroll: true,
            onSuccess: closeForm,
        });
    }
...
function destroy(id) {
    ...
    router.delete(route('admin.traits.destroy', id), { preserveScroll: true });
}
</script>
...
<template>
    <div>
        <div class="page-header">
            <h1 class="page-title">Черты характера</h1>
        ...
        <div class="table-wrap">
            <table class="data-table">
                ...
                <tbody>
                    <tr v-for="trait in traits" :key="trait.id">
                        <td>{{ trait.name_ru }}</td>
                        <td>{{ trait.users_count }}</td>
                        <td>
                            <div class="actions">
                                <button class="btn-edit" @click="openEdit(trait)">Изменить</button>
                                <button class="btn-danger" @click="destroy(trait.id)">Удалить</button>
                            </div>
                        </td>
                    </tr>
                    <tr v-if="!traits.length">
                        <td colspan="3" class="empty-row">Черт нет</td>
                    </tr>
                </tbody>
            </table>
        </div>
            ...
            <div v-if="showForm" class="overlay" @click.self="closeForm">
                <div class="modal">
                    <div class="modal__header">
                        <span>{{ editingId ? 'Редактировать черту' : 'Новая черта' }}</span>
