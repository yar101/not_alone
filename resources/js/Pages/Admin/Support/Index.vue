...
const sentMsgIds = new Set(); // skip own messages in Echo
    ...
    if (window.Echo) {
        echoChannel = window.Echo.private('admin.support.' + conv.id)
            .listen('.message.sent', (data) => {
                ...
                // Handle close/open state changes
                if (data.type === 'system') {
...
async function onFileSelected(e) {
    ...
    try {
        ...
        const msgRes = await axios.post(route('admin.support.image', activeConv.value.id), { image_url: imageUrl });
            ...
            <div class="support-sidebar__list">
                ...
                >
                    ...
                    <div class="conv-item__meta">
                        ...
                        <span v-if="conv.closed_at" class="conv-item__closed-badge">🔒</span>
                    </div>
                </div>

                <div v-if="conversations.length === 0" class="conv-empty">Нет чатов</div>
            </div>
        </aside>
